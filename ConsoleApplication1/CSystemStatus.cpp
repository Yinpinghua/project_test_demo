...
void CSystemStatus::SystemInit(DWORD object)
{
	...
	if (object & SYSSTATE_CPU_USAGE)	//CPU使用率
		state = PdhAddCounter(m_Query, L"\\Processor(_Total)\\% Processor Time", NULL, &m_CpuTotal);
	if (object & SYSSTATE_DISK_READ)	//磁盘读速度
		state = PdhAddCounter(m_Query, L"\\PhysicalDisk(_Total)\\Disk Read Bytes/sec", NULL, &m_DiskRead);
	if (object & SYSSTATE_DISK_WRITE)	//磁盘写速度
		state = PdhAddCounter(m_Query, L"\\PhysicalDisk(_Total)\\Disk Write Bytes/sec", NULL, &m_DiskWrite);
	if (object & SYSSTATE_NET_DOWNLOAD)//网络下载速度
		state = PdhAddCounter(m_Query, L"\\Network Interface(Realtek PCIe GbE Family Controller)\\Bytes Received/sec", 0, &m_NetDownload);
		//state = PdhAddCounter(m_Query, L"\\Network Interface(killer E2200 Gigabit Ethernet Controller)\\Bytes Received/sec", NULL, &m_NetDownload);
	if (object & SYSSTATE_NET_UPLOAD)	//网络上传速度
		state = PdhAddCounter(m_Query, L"\\Network Interface(Realtek PCIe GbE Family Controller)\\Bytes Sent/sec", 0, &m_NetUpload);
		//state = PdhAddCounter(m_Query, L"\\Network Interface(killer E2200 Gigabit Ethernet Controller)\\Bytes Sent/sec", NULL, &m_NetUpload);

	state = PdhCollectQueryData(m_Query);
	std::this_thread::sleep_for(std::chrono::milliseconds(500));             //这里要有延时不然结果相当不准确  
	state = PdhCollectQueryData(m_Query);
}
...
#endif
	TCHAR szDisk[3] = { 0 };
	wsprintf(szDisk,L"%c:", strDirve[0]);
	bResult = GetDiskFreeSpaceEx(
		szDisk,
...
void CSystemStatus::GetCPUid(std::string& CPUid)
{
	...
	CPUid.resize(50);
	sprintf(&CPUid[0], "%08X%08X%08X%08X", s1, s2, s3, s4);
}
